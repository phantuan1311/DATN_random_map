...
var wallTiles = {
	...
	"topLeftCorner": Vector2(15,8),
	"topRightCorner": Vector2(12,9),
	"bottomLeftCorner": Vector2(10,8),
	"bottomRightCorner": Vector2(9,8),

	#"topLeftCornerReverse": Vector2(2,6),
	#"topRightCornerReverse": Vector2(0,6),
	"bottomLeftCornerReverse": Vector2(7,7),
	"bottomRightCornerReverse": Vector2(12,7),
}
...
func _create_top_left_corner(position):
	...
	if _has_floor(position + directions["left"] + directions["bottom"]):
		#wall_layer.set_cell(position + directions["top"] + directions["left"], 0, wallTiles["topLeftCorner"])
		#wall_layer.set_cell(position + directions["top"] * 2 + directions["left"], 0, wallTiles["topLeftCornerReverse"])
		
		wall_layer.set_cell(position + directions["top"] + directions["left"], 0, wallTiles["topLeftCorner"])
		wall_layer.set_cell(position + directions["top"] * 2 + directions["left"], 0)
	else:
		wall_layer.set_cell(position + directions["top"] + directions["left"], 0, wallTiles["topLeftCorner"])
...
func _create_top_right_corner(position):
	...
	if _has_floor(position + directions["right"] + directions["bottom"]):
		#wall_layer.set_cell(position + directions["top"] + directions["right"], 0, wallTiles["topRightCorner"])
		#wall_layer.set_cell(position + directions["top"] * 2 + directions["right"], 0, wallTiles["topRightCornerReverse"])
		
		wall_layer.set_cell(position + directions["top"] + directions["right"], 0, wallTiles["topRightCorner"])
		wall_layer.set_cell(position + directions["top"] * 2 + directions["right"], 0)
	else:
		wall_layer.set_cell(position + directions["top"] + directions["right"], 0, wallTiles["topRightCorner"])
...
func _create_left_wall(position):
	var left = position + directions["left"]
	if wall_layer.get_cell_source_id(left) == -1:
		var top = position + directions["top"]
		if _has_floor(top + directions["left"]):
			wall_layer.set_cell(left, 0, wallTiles["bottomLeftCornerReverse"])
		else:
			wall_layer.set_cell(left, 0, wallTiles["left"])
...
func _create_right_wall(position):
	var right = position + directions["right"]
	if wall_layer.get_cell_source_id(right) == -1:
		var top = position + directions["top"]
		if _has_floor(top + directions["right"]):
			wall_layer.set_cell(right, 0, wallTiles["bottomRightCornerReverse"])
		else:
			wall_layer.set_cell(right, 0, wallTiles["right"])
